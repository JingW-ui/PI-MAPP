...
> 基于 Python 的交互式学科排名 系统，支持 5570 条学科排名记录的动态筛选、搜索与可视化。
> 本来最好的方式是做一个网站的 但是要服务器 还是做本地的UI 方便一点！
> 主要是官网做的太丑了，且不会显示学校的级别（985、211），以及所在地。
> 该UI可根据学科门类、评估等级、学校名称、学校级别、所在地、学科名称进行筛选。
...
##
```commandline
pyinstaller -F -w -i app.ico ^
--add-data "app.ico;." ^
--add-data "univ_province_city.csv;." ^
--add-data "学科评估结果_第四轮.csv;." ^
--name="subject_ranking_1.0.0" ^
subject_ranking_app.py
```
